...
func (r *run) newCacheFs(t *testing.T, remote, id string, needRemote, purge bool, cfg map[string]string, flags map[string]string) (fs.Fs, *cache.Persistent) {
	...
	if !remoteExists {
		...
		if remoteType != "cache" {
			if remoteType == "crypt" {
				rootIsCrypt = true
				fs.ConfigFileSet(remote, "password", cryptPassword1)
				fs.ConfigFileSet(remote, "password2", cryptPassword2)
			}
...
func (r *run) writeObjectReader(t *testing.T, f fs.Fs, remote string, in io.Reader) fs.Object {
	modTime := time.Now()
	objInfo := fs.NewStaticObjectInfo(remote, modTime, -1, true, nil, f)
...
func (r *run) updateData(t *testing.T, rootFs fs.Fs, src, data, append string) error {
	...
	if r.useMount {
		...
		objInfo1 := fs.NewStaticObjectInfo(src, time.Now(), int64(len(data1)), true, nil, rootFs)
